<?php

namespace Aleedhillon\MetaTraderClient\Lib;

//+------------------------------------------------------------------+
//|                                             MetaTrader 5 Web API |
//|                   Copyright 2000-2021, MetaQuotes Software Corp. |
//|                                        http://www.metaquotes.net |
//+------------------------------------------------------------------+
/**
 * Crypt aes 256
 */
class MT5CryptAes256
{
    //--- The number of 32-bit words comprising the plaintext and columns comrising the state matrix of an AES cipher.
    private static $nb = 4;
    // The number of 32-bit words comprising the cipher key in this AES cipher.
    private $nk;
    //--- The number of rounds in this AES cipher.
    private $nr;
    //--- The S-Box substitution table.
    private static $sbox = array();
    //--- The inverse S-Box substitution table.
    private static $invSbox = array();
    // Log table based on 0xe5
    private static $ltable = array();
    //--- Inverse log table
    private static $atable = array();
    //--- The key schedule in this AES cipher.
    private $cipher;
    //--- The state matrix in this AES cipher with Nb columns and 4 rows
    private $cipherTable;
    //--- Determines the lenght of key z
    private $keyLength;

    /**
     * Constructs an AES cipher using a specific key.
     *
     * @param $key
     */
    public function __construct($key, $len = 32)
    {
        $this->Init($key, $len);
    }

    /**
     * initialization key
     *
     * @param $aesKey
     *
     */
    public function Init($aesKey, $len = 32)
    {
        $this->nk = $len / 4;
        $this->nr = $this->nk + self::$nb + 2;
        if ($this->nk != 4 && $this->nk != 6 && $this->nk != 8)
            return ''; //("Key is " . ($this->nk * 32) . " bits long. *not* 128, 192, or 256.");
        $this->nr = $this->nk + self::$nb + 2;
        $this->cipher = array(); // Nb*(Nr+1) 32-bit words
        $this->cipherTable = array(array()); // 2-D array of Nb colums and 4 rows
        $this->KeyExpansion($aesKey); // places expanded key in w
    }

    /** Encrypts the 16-byte plain text.
     *
     * @param   string $key
     *
     * @return  $x 16-byte ciphertext
     * @return string
     */
    public function encryptBlock($x)
    {
        $y = ''; // 16-byte string
        // place input x into the initial state matrix in column order
        for ($i = 0; $i < 4 * self::$nb; $i++) {
            // we want integerger division for the second index
            $this->cipherTable[$i % 4][($i - $i % self::$nb) / self::$nb] = ord($x[$i]);
        }
        //--- add round key
        $this->addRoundKey(0);
        //---
        for ($i = 1; $i < $this->nr; $i++) {
            //--- substitute bytes
            $this->subBytes();

            //--- shift rows
            $this->shiftRows();

            //--- mix columns
            $this->mixColumns();
            //--- add round key
            $this->addRoundKey($i);
        }

        //--- substitute bytes
        $this->subBytes();
        //--- shift rows
        $this->shiftRows();
        //--- add round key
        $this->addRoundKey($i);
        //--- place state matrix s into y in column order
        for ($i = 0; $i < 4 * self::$nb; $i++) {
            $y .= chr($this->cipherTable[$i % 4][($i - $i % self::$nb) / self::$nb]);
        }
        return $y;
    }

    public function __destruct()
    {
        unset($this->cipher);
        unset($this->cipherTable);
    }

    /** makes a big key out of a small one
     * @return void
     **/
    private function KeyExpansion($key)
    {
        // Rcon is the round constant
        static $rcon = array(0x00000000, 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000, 0x6c000000, 0xd8000000, 0xab000000, 0x4d000000, 0x9a000000, 0x2f000000);
        // the first Nk words of w are the cipher key z
        for ($i = 0; $i < $this->nk; $i++) {
            $this->cipher[$i] = 0;
            // fill an entire word of expanded key cipher
            // by pushing 4 bytes into the cipher[i] word
            $this->cipher[$i] = ord($key[4 * $i]); // add a byte in
            $this->cipher[$i] <<= 8; // make room for the next byte
            $this->cipher[$i] += ord($key[4 * $i + 1]);
            $this->cipher[$i] <<= 8;
            $this->cipher[$i] += ord($key[4 * $i + 2]);
            $this->cipher[$i] <<= 8;
            $this->cipher[$i] += ord($key[4 * $i + 3]);
        }
        //---
        for (; $i < self::$nb * ($this->nr + 1); $i++) {
            $temp = $this->cipher[$i - 1];
            //---
            if ($i % $this->nk == 0)
                $temp = $this->subWord($this->rotWord($temp)) ^ $rcon[$i / $this->nk];
            else if ($this->nk > 6 && $i % $this->nk == 4)
                $temp = $this->subWord($temp);
            //---
            $this->cipher[$i] = $this->cipher[$i - $this->nk] ^ $temp;
            //---
            self::make32BitWord($this->cipher[$i]);
        }
    }

    /**
     * adds the key schedule for a round to a state matrix.
     * @return void
     *
     * @param $round
     */
    private function addRoundKey($round)
    {
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$nb; $j++) {
                //--- place the i-th byte of the j-th word from expanded key w into temp
                $temp = $this->cipher[$round * self::$nb + $j] >> (3 - $i) * 8;
                //--- Cast temp from a 32-bit word into an 8-bit byte.
                $temp %= 256;
                //-- Can't do unsigned shifts, so we need to make this temp positive
                $temp = ($temp < 0 ? (256 + $temp) : $temp);
                //---
                $this->cipherTable[$i][$j] ^= $temp; // xor temp with the byte at location (i,j) of the state
            }
        }
    }

    /**
     * unmixes each column of a state matrix.
     * @return void
     **/
    private function invMixColumns()
    {
        //--- There are nb columns
        for ($i = 0; $i < self::$nb; $i++) {
            $s0 = $this->cipherTable[0][$i];
            $s1 = $this->cipherTable[1][$i];
            $s2 = $this->cipherTable[2][$i];
            $s3 = $this->cipherTable[3][$i];
            //---
            $this->cipherTable[0][$i] = $this->mult(0x0e, $s0) ^ $this->mult(0x0b, $s1) ^ $this->mult(0x0d, $s2) ^ $this->mult(0x09, $s3);
            $this->cipherTable[1][$i] = $this->mult(0x09, $s0) ^ $this->mult(0x0e, $s1) ^ $this->mult(0x0b, $s2) ^ $this->mult(0x0d, $s3);
            $this->cipherTable[2][$i] = $this->mult(0x0d, $s0) ^ $this->mult(0x09, $s1) ^ $this->mult(0x0e, $s2) ^ $this->mult(0x0b, $s3);
            $this->cipherTable[3][$i] = $this->mult(0x0b, $s0) ^ $this->mult(0x0d, $s1) ^ $this->mult(0x09, $s2) ^ $this->mult(0x0e, $s3);
        }
    }

    /**
     * applies an inverse cyclic shift to the last 3 rows of a state matrix.
     * @return void
     **/
    private function invShiftRows()
    {
        $temp = "";
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$nb; $j++)
                $temp[($i + $j) % self::$nb] = $this->cipherTable[$i][$j];
            for ($j = 0; $j < self::$nb; $j++)
                $this->cipherTable[$i][$j] = $temp[$j];
        }
    }

    /**
     * applies inverse S-Box substitution to each byte of a state matrix.
     * @return void
     **/
    private function invSubBytes()
    {
        for ($i = 0; $i < 4; $i++)
            for ($j = 0; $j < self::$nb; $j++)
                $this->cipherTable[$i][$j] = self::$invSbox[$this->cipherTable[$i][$j]];
    }

    /**
     * mixes each column of a state matrix.
     * @return void
     **/
    private function mixColumns()
    {
        //--- There are nb columns
        for ($i = 0; $i < self::$nb; $i++) {
            $s0 = $this->cipherTable[0][$i];
            $s1 = $this->cipherTable[1][$i];
            $s2 = $this->cipherTable[2][$i];
            $s3 = $this->cipherTable[3][$i];
            //---
            $this->cipherTable[0][$i] = $this->mult(0x02, $s0) ^ $this->mult(0x03, $s1) ^ $this->mult(0x01, $s2) ^ $this->mult(0x01, $s3);
            $this->cipherTable[1][$i] = $this->mult(0x01, $s0) ^ $this->mult(0x02, $s1) ^ $this->mult(0x03, $s2) ^ $this->mult(0x01, $s3);
            $this->cipherTable[2][$i] = $this->mult(0x01, $s0) ^ $this->mult(0x01, $s1) ^ $this->mult(0x02, $s2) ^ $this->mult(0x03, $s3);
            $this->cipherTable[3][$i] = $this->mult(0x03, $s0) ^ $this->mult(0x01, $s1) ^ $this->mult(0x01, $s2) ^ $this->mult(0x02, $s3);
        }
    }

    /**
     * applies a cyclic shift to the last 3 rows of a state matrix.
     * @return void
     **/
    private function shiftRows()
    {
        $temp = array();
        for ($i = 1; $i < 4; $i++) {
            for ($j = 0; $j < self::$nb; $j++) {
                $temp[$j] = $this->cipherTable[$i][($j + $i) % self::$nb];
            }
            for ($j = 0; $j < self::$nb; $j++)
                $this->cipherTable[$i][$j] = $temp[$j];
        }
    }

    /**
     * applies S-Box substitution to each byte of a state matrix.
     * @return void
     **/
    private function subBytes()
    {
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$nb; $j++)
                $this->cipherTable[$i][$j] = self::$sbox[$this->cipherTable[$i][$j]];
        }
    }

    /**
     * multiplies two polynomials a(x), b(x) in GF(2^8) modulo the irreducible polynomial m(x) = x^8+x^4+x^3+x+1
     *
     * @param $a
     * @param $b
     *
     * @return int 8-bit value
     */
    private static function mult($a, $b)
    {
        $sum = self::$ltable[$a] + self::$ltable[$b];
        $sum %= 255;
        //--- Get the antilog
        $sum = self::$atable[$sum];
        return ($a == 0 ? 0 : ($b == 0 ? 0 : $sum));
    }

    /**
     * applies a cyclic permutation to a 4-byte word.
     *
     * @param string $w
     *
     * @return int
     */
    private static function rotWord($w)
    {
        $temp = $w >> 24; // put the first 8-bits into temp
        $w <<= 8; // make room for temp to fill the lower end of the word
        self::make32BitWord($w);
        //--- Can't do unsigned shifts, so we need to make this temp positive
        $temp = ($temp < 0 ? (256 + $temp) : $temp);
        $w += $temp;
        //---
        return $w;
    }

    /**
     * applies S-box substitution to each byte of a 4-byte word.
     *
     * @param string $w
     *
     * @return int
     */
    private static function subWord($w)
    {
        //--- loop through 4 bytes of a word
        for ($i = 0; $i < 4; $i++) {
            $temp = $w >> 24; // put the first 8-bits into temp
            //--- Can't do unsigned shifts, so we need to make this temp positive
            $temp = ($temp < 0 ? (256 + $temp) : $temp);
            $w <<= 8; // make room for the substituted byte in w;
            self::make32BitWord($w);
            $w += self::$sbox[$temp]; // add the substituted byte back
        }
        //---
        self::make32BitWord($w);
        //---
        return $w;
    }

    /**
     * reduces a 64-bit word to a 32-bit word
     *
     * @param int $w
     *
     * @return void
     */
    private static function make32BitWord(&$w)
    {
        // Reduce this 64-bit word to 32-bits on 64-bit machines
        $w &= 0x00000000FFFFFFFF;
    }
}
